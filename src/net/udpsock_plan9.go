...
func listenMulticastUDP(ctx context.Context, network string, ifi *Interface, gaddr *UDPAddr) (*UDPConn, error) {
	...
	for _, addr := range addrs {
		_, err = l.ctl.WriteString("addmulti " + addr.String() + " " + gaddr.IP.String())
		if err != nil {
			return nil, err
		}
	}
	...
	fd, err := l.netFD()
	if fd == nil {
		return nil, errors.New("netFD is nil")
	}
	udpCon := newUDPConn(fd)
	if udpCon == nil {
		return nil, errors.New("udpCon is nil")
	}
	return udpCon, err
}
